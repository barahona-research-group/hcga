...
def extract_benchmark_graphs(datadir, dataname, max_nodes=None):
    ...
    with open(prefix + "_A.txt") as f:
        adj_list = pd.read_csv(f,sep=', ', delimiter=None, dtype=np.int, header=None).to_numpy()#.flatten()
    ...
    if Path(prefix + "_node_attributes.txt").exists():
        ...
        node_features = [[node_label] + list(node_attribute) for node_label, node_attribute in zip(node_labels, node_attributes)]
    else:
