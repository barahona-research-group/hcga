...
def plot_dendogram_top_features(X, top_features_list, top_feat_indices, image_folder, top_N=40):
    df_topN = pd.DataFrame(
            columns = top_features_list[:top_N], 
            data=X[:,top_feat_indices[:top_N]])            
    ...
    dn = dendrogram(Z)         
    plt.savefig(
            image_folder+'/endogram_top{}_features.svg'.format(top_N), 
            bbox_inches = 'tight') 
...
def top_features_importance_plot(g,X,top_feat_indices,feature_names,y, name = 'xgboost', image_folder='images'):
    """ 
    ...
    plt.colorbar(sc)
    plt.savefig(image_folder + '/scatter_top2_feats_'+g.dataset+'_'+name+'.svg', bbox_inches = 'tight') 

def plot_violin_feature(g,X,y,feature_id,feature_names, name = 'xgboost', image_folder='images'):   
    """
    ...
    plt.savefig(image_folder + '/violin_plot_'+g.dataset+'_'
            +str(feature_names[feature_id])+'_'+name+'.svg', bbox_inches = 'tight') 
