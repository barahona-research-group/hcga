...
class TestingGraphMethods(unittest.TestCase):
    def setUp(self):
        ...
        self.g = hcga_graphs.Graphs(
            graphs=rgraphs, graph_class=rlabels, dataset="testing"
        )
    ...
    def test_plot_features_classification(self):
        ...
        final_acc = self.g.graph_classification(
            reduc_threshold=0.5, plot=True, image_folder=output_dir
        )
...
def synthetic_data_sbm(N=1000, seed=None):
    ...
    for i in range(int(N / 2)):
        G = nx.stochastic_block_model(
            ...
            [
                [prob_in, prob_out, prob_out],
                [prob_out, prob_in, prob_out],
                [prob_out, prob_out, prob_in],
            ],
        )
