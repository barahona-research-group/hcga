...
#                    graphs[i] = Gc
    ...
    def graph_classification(self,plot=True,ml_model='xgboost', data='all', reduc_threshold = 0.9, image_folder='images'):
        ...
        top_features_list, top_feat_indices = hcga_analysis.compute_top_features(X,top_feats,feature_names)

        self.top_feats = top_feats         
        self.top_features_list = top_features_list       

        if plot:
            hcga_plotting.plot_top_features(X,top_feats, feature_names, image_folder = image_folder, threshold = reduc_threshold)
        ...
        X_reduced, top_feat_indices = hcga_analysis.reduce_feature_set(X,top_feats, threshold = reduc_threshold)        
        testing_accuracy_reduced_set, top_feats_reduced_set = hcga_analysis.classification(X_reduced,y,ml_model) 
        ...
        if plot:
            hcga_plotting.top_features_importance_plot(self, X,top_feat_indices,feature_names,y, name='xgboost', image_folder = image_folder)
            hcga_plotting.plot_violin_feature(self,X,y,top_feat_indices[0],feature_names, name='xgboost', image_folder = image_folder)
        ...
        if plot:
            hcga_plotting.top_features_importance_plot(self,
                X_reduced,top_feat_index[0:2],
                feature_names_reduced,y, name='univariate', 
                image_folder = image_folder)
            hcga_plotting.plot_violin_feature(self,
